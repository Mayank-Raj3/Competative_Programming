...
void solve() {
	int n , q ; cin >> n >> q ;
	vector<int> ans(n + 2), pred(n + 2), predi(n + 2);
	while (q--) {
		int a , d, l , r  ; cin >> a >> d >> l >> r ;
		int temp = mod_sub(a, mod_mul(l, d, mod), mod);
		predi[l] = mod_add(predi[l], d, mod);
		predi[r + 1] = mod_sub(predi[r + 1], d, mod);


		pred[l] = mod_add(pred[l], temp, mod);
		pred[r + 1] = mod_sub(pred[r + 1], temp, mod);

	}

	for (int i = 1 ; i <= n ; i++) {
		if (i) {
			predi[i] = mod_add(predi[i], predi[i - 1], mod);
			pred[i] = mod_add(pred[i], pred[i - 1], mod);
		}
	}

	for (int i = 1; i <= n ; i++) {
		cout << mod_add(mod_mul(predi[i], i, mod), pred[i], mod) << " ";
	}
}
