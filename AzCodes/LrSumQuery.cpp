...
void solve() {
	...
	for (int i = 1 ; i <= n  ; i++ ) brr[i] = (arr[i] * (i )) % mod;
	for (int i = 1 ; i <= n  ; i++ ) {
		arr[i] = ((arr[i] % mod ) + (arr[i - 1] % mod)) % mod;
	}

	for (int i = 1 ; i <= n  ; i++ ) {
		brr[i] = ((brr[i] % mod ) + (brr[i - 1] % mod) + mod) % mod;
	...
	while (q--) {
		int l , r ; cin >> l >> r ;
		int a  = (brr[r] - brr[l - 1]) % mod;
		int b  = (((1 - l) % mod) * (arr[r] - arr[l - 1]) % mod) % mod;
		cout << mod_add(a, b, mod) << nline;
	}
