...
void solve() {
	int n , q , k ; cin >> n >> q >> k ;
	vector<int> arr(n + 1), brr(n + 1), kk(n + 1);
	...
	for (int i = 1 ; i <= n; i++) {
		brr[i] = mod_mul(arr[i], kk[i], mod);
	}
	for (int i = 1 ; i <= n; i++) {
		brr[i] = mod_add(brr[i], brr[i - 1], mod);
	}
	db(brr)
	while (q--) {
		int l , r ; cin >> l >> r ;
		int ans = mod_sub(brr[r] , brr[l - 1], mod);
		ans = mod_mul( ans , binpow(kk[l], mod - 2, mod), mod) ;
		cout << ans << nline;
	}
}
