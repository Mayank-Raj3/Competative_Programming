...
void solve() {
	int n, m, q;
	cin >> n >> m >> q;
	vector<vector<int>> arr(n + 2, vector<int>(m + 2, 0)), partial(n + 2, vector<int>(m + 2, 0));
	while (q--) {
		int x1, y1, x2, y2, c;
		cin >> x1 >> y1 >> x2 >> y2 >> c;
		arr[x1][y1] += c;
	...
	int maxi = arr[1][1], freq = 1;
	map<int, int> mpp;
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= m; j++) {
			partial[i][j] = arr[i][j] + partial[i - 1][j] + partial[i][j - 1] - partial[i - 1][j - 1];
			mpp[partial[i][j]]++;
		}
	}
