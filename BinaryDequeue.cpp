...
void solve() {
	...
	for (int i = 0 ; i < n; i++) {
		sum += arr[i];//pick
		while (sum - arr[i] > s) {
			sum -= arr[j++];//revove only if after removing sum>s
		}
		if (sum == s) {
			maxi = max(maxi, (i - j + 1));
		}
	}
	// agr ham array ki size maximize karenge tho obv operation kam honge
	// number of operation will be n - lenght of array
	if (s > sum) {
		cout << -1 << nline;
		return ;
	}
