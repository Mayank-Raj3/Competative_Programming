...
void solve() {
	...
	vector<vector<int>> divine(n + 2, vector<int>(n + 1));
	divine[1] = arr;
	for (int i = 2 ; i <= n + 1; i++) {
		auto prev = divine[i - 1];
		vector<int> cnt(n + 1, 0);
		for (int j = 1 ; j <= n ; j++) {
			cnt[prev[j]]++;
		}
		for (int j = 1 ; j <= n ; j++) {
			divine[i][j] = cnt[prev[j]];
		}
	}
	db(divine);
	int m ; cin >> m ;
	while (m--) {
		int k , x ; cin >> x >> k ;
		if (k == 0) {
			cout << divine[1][x] << nline;
			continue ;
		}
		else if (k >= n ) {
			cout << divine[n + 1 ][x] << nline;
		} else {
			cout << divine[k + 1][x] << nline;
		}
