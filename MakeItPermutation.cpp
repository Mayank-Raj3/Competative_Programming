...
void solve() {
	...
	vector<int> arr(n);
	set<int> uq ;
	for (int i = 0 ; i < n; i++) {cin >> arr[i]; uq.insert(arr[i]);}
	int m = uq.size();
	int result = 1e18 ;
	vector<int> newarr(uq.begin(), uq.end());
	db(newarr);
	for (int i = 0 ; i < m ; i++) {
		int x = newarr[i] - i - 1; // add
		int y = m - i - 1; // remove;
		result = min(result, (((n - m ) * c ) + (d * x) + ( c * y)));
	}
	result = min(result, ((n * c) + d));
	cout << result << nline;
}
int32_t main() {
#ifndef ONLINE_JUDGE
