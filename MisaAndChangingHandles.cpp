...
void solve() {
	...
	for (int  i = 0; i < n; i++) {
		string s , t ;
		cin >> s >> t;
		// agr nhi ha tho possible ans ha
		if (m.find(s) == m.end()) m[t] = mp(s, t);
		else {
			// else connect karo string ko and them push it back, aut purane vale ko erase
			pair<string, string> x = m[s];
			m.erase(s);
			m[t] = mp(x.ff, t);
		}
	}
