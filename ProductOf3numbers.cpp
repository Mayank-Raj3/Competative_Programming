...
void solve() {
	...
	//when we have only one factor
		...
		auto m1 = facts.begin()->ff;
		auto m2 = facts.rbegin()->ff;
		int a = facts[m1] + facts[m2];
		if (a >=  4) {
			YES
			cout << m1 << " " << m2 << " ";
			cout << temp / (m1 * m2) << nline;
		} else {
			NO
		}
