...
void solve() {
	...
	auto check = [&](int mid) {
		...
		while (city < n && tow < m) {
			if (a[city] < range[tow].ff) {
				return false;
			}//city cover he nhi ho rha tho
			if (a[city] > range[tow].ss) {
				tow++;
			}//city aage ha tow k tho next tower
			else {//covered city
				city++;
			}
		}
		return city == n;
//agr sare city cover ho gye tho
	};
	while (l <= r) {
		int mid = (l + r) / 2;
		if (check(mid)) {
			//NNNNNNYYYYYYYYY type monotone space
			ans  = mid ;
			r = mid - 1;
		} else {
			l = mid + 1;
